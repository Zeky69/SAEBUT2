  ...
  exports.getCategorie = async (req, res) => {
    ...
    exports.getCategorieByProduct = async (req, res) => {
      ...
      exports.createCommandeWithoutAccount = async (req, res) => {
        ...
        try {
          const id_user = await userService.registerGhostsUser(acheteur.nom,acheteur.prenom,acheteur.email)
          const id_commande = await shopService.setCommande(id_user)
          for (const element of panier) {
            if (element.header.type !== "billet") {
              await shopService.setCommandeLineArticle(id_commande, element.id, element.quantity)
            }
          }
          let billets = []
          if (billet && billet.length > 0) {
              for (const element of billet) {
                let uuid =await shopService.setCommandeLineBillet(id_commande, element.item.id, element.item.subId,element.nom ,element.prenom, {date :element.item.date})
                billets.push({uuid:uuid, id:element.item.id , subId:element.item.subId, nom:element.nom, prenom:element.prenom, date:element.date})
              }
            }
