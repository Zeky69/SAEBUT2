  ...
  <l-map style="width:800px ; height: 500px" :min-zoom="18" :max-zoom="21" :max-bounds="cartebounds" :center="center">
    ...
    <l-image-overlay :url="'../map2D.png'" :bounds="cartebounds"></l-image-overlay>
      ...
      >
        ...
        <info-panel v-if="featureSelected!=null" @edit-feature="updateFeatures"  @ask="askGetEmp" @free="freeMyEmp" @close-panel="closePanel" :providers="providers"  :prestataire="prestataireObject" :feature="featureSelected"> </info-panel>
...
export default {
  ...
  computed: {
    ...mapState(['user_id','prestataireObject'])
  },
  async created() {
  ...
  methods: {
      ...
      updateInfoEmpPrestataire(feature.properties.id
        ...
        if (response.length > 0) {
          for (let i = 0; i < this.features.length; i++) {
            if (this.features[i].properties.id === response[0].id_emplacement) {
              this.features[i].properties.name = response[0].nom;
              this.features[i].properties.description = response[0].description;
            }
          ...
          (response) => {
            if (response.length > 0){
              this.features.forEach((feat)=>{
                if(feat.properties.id === response[0].id_emplacement){
                  feat.properties.apartient = response[0].prestataire_id;
                  feat.properties.accept = false;
                }
              })
              this.couleur = []
              this.features.forEach((feature)=>{
    ...
    freeMyEmp(feature){
      freeEmp(feature).then(
          (response) => {
            if(response.length > 0){
              this.features.forEach((feature)=>{
                if(feature.properties.id ===response[0].id_emplacement){
                  feature.properties.apartient = null;
                  feature.properties.accept = false;
                }
              })
              this.couleur = []
              this.features.forEach((feature)=>{
    ...
    polygonOption(feature){
      ...
      const isAccept = feature.properties.accept;
      console.log("isPrestataire", isPrestataire)
      console.log("isAccept", isAccept)
      const isNullApartient = feature.properties.apartient === null;

      return isPrestataire && isAccept  ? '#4be500' :
          isPrestataire && !isAccept ? '#fff51d' :
