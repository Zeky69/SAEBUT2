...
function loadfinal(){
    loader.load('public/map_try2.fbx', (loadedFbx) => {
            ...
            for (var i = 0; i < children.length; i++) {
                if (children[i].name.slice(-3) == "bat"){
                    ...
                    if (children[i].name.slice(-3) == "emp") {
                        ...
                        const texturesol = new THREE.TextureLoader().load('public/grass.jpg');
...
const batidInput = document.getElementById("batidInput");
...
function start(){
    ...
    loadfinal();
    setup();
    const texture_emp = new THREE.TextureLoader().load('public/tex_socle.jpg');
    const texture_bat = new THREE.TextureLoader().load('public/albedo.jpg');
...
function onMouseClick(event) {
    ...
    const originmat = selectedObject.material.clone();
    const origineColor = selectedObject.material.color.getHex();
